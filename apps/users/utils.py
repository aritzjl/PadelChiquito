...
async def send_email_token(email, token):
    try:
        ...
        email_body = f'Clica aqui para verificar tu cuenta: http://127.0.0.1:8000/verify/{token}'
...
async def send_recovery_mail(email, token):
    try:
        ...
        email_body = f'Clica este link para obtener una nueva contrasena: http://127.0.0.1:8000/change-password/{token}'
